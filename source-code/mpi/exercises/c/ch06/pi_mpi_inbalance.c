...
int main(int argc, char *argv[]) {
    ...
    /* create inbalance */
    if (nr_procs > 3) {
        if (my_rank == nr_procs - 1) {
            MPI_Send(&range.ub, 1, MPI_INT, nr_procs - 2, 0, MPI_COMM_WORLD);
            range.ub = range.lb - 1;
        } else if (my_rank == nr_procs - 2) {
            MPI_Recv(&range.ub, 1, MPI_INT, nr_procs - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
    }
